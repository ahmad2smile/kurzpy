...
def get_class_info[TModel: SQLModel](model_class: type[TModel]) -> tuple[str, type[Any], str]:
    ...
    if id_field is None or not id_field.annotation:
        raise Exception(f"An id field is required to use {inspect.stack()[1][3]} on {class_name}")
